...
public class Wastetree1Structure extends FalloutWastelandsModElements.ModElement {
	private static Feature<NoFeatureConfig> feature = null;
	private static ConfiguredFeature<?, ?> configuredFeature = null;
	public Wastetree1Structure(FalloutWastelandsModElements instance) {
		super(instance, 178);
	...
	private static class FeatureRegisterHandler {
		...
		public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
					...
					if ((random.nextInt(1000000) + 1) <= 8000) {
						...
						for (int a = 0; a < count; a++) {
							...
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("fallout_wastelands", "wastelandtree1"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("wastetree_1"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("fallout_wastelands:wastetree_1"), configuredFeature);
		}
	}
	@SubscribeEvent
