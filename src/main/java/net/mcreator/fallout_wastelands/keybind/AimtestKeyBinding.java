...
public class AimtestKeyBinding extends FalloutWastelandsModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;

	public AimtestKeyBinding(FalloutWastelandsModElements instance) {
		super(instance, 605);
	...
	public void initElements() {
		keys = new KeyBinding("key.fallout_wastelands.aimtest", GLFW.GLFW_KEY_SCROLL_LOCK, "key.categories.gameplay");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					FalloutWastelandsMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
	...
	private static void pressAction(PlayerEntity entity, int type, int pressedms) {
		...
		if (type == 0) {
			...
			AimtestOnKeyPressedProcedure.executeProcedure(Stream.of(new AbstractMap.SimpleEntry<>("entity", entity)).collect(HashMap::new,
