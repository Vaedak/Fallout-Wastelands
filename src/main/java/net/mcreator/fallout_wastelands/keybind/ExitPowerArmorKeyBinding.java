...
public class ExitPowerArmorKeyBinding extends FalloutWastelandsModElements.ModElement {
	...
	public void initElements() {
		keys = new KeyBinding("key.fallout_wastelands.exit_power_armor", GLFW.GLFW_KEY_N, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					...
					FalloutWastelandsMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(1, dt));
